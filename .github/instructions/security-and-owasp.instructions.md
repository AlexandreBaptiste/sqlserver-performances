...
- **Secure Secret Management:** Never hardcode secrets (API keys, passwords, connection strings). Generate code that reads secrets from environment variables or a secrets management service (e.g., HashiCorp Vault, AWS Secrets Manager, Azure Key Vault). Include a clear placeholder and comment.
  ...
  var apiKey = "sk_this_is_a_very_bad_idea_12345";
  ```
...
- **Prevent Cross-Site Scripting (XSS):** When generating frontend code that displays user-controlled data, use context-aware output encoding. Prefer methods that treat data as text by default over those that parse HTML. When HTML is necessary, sanitize with a trusted library first.
...
- **Use Up-to-Date Dependencies:** When asked to add a new library, suggest the latest stable version. Remind the user to run vulnerability scanners (e.g., `dotnet list package --vulnerable`, `npm audit`, Snyk) to check for known vulnerabilities in project dependencies.
...
- **Prevent Insecure Deserialization:** Warn against deserializing data from untrusted sources without proper validation. If deserialization is necessary, recommend formats that are less prone to attack and implement strict type checking.

## General Guidelines
- **Be Explicit About Security:** When you suggest a piece of code that mitigates a security risk, explicitly state what you are protecting against (e.g., "Using a parameterized query here to prevent SQL injection.").
- **Educate During Code Reviews:** When you identify a security vulnerability in a code review, not only provide the corrected code but also explain the risk associated with the original pattern.
- **Defense in Depth:** Apply multiple layers of security controls rather than relying on a single mechanism.
- **Input Validation**: Validate all inputs at the boundary of your system. Never trust input from external sources.
- **Output Encoding**: Encode all output that may contain untrusted data before rendering it.
- **Logging Without Sensitive Data**: Log security events but never log sensitive information like passwords, tokens, or PII.
