...
- **Usings**: Start with `using Microsoft.Playwright;` and either `using Microsoft.Playwright.Xunit;`, `using Microsoft.Playwright.NUnit;`, or `using Microsoft.Playwright.MSTest;`.
- **Organization**: Create test classes that inherit from `PageTest`. Group related tests for a feature in the same test class.
...
public class MovieSearchTests : PageTest
{
    ...
    public async Task SearchForMovieByTitle()
    {
        ...
        {
            await Expect(Page.GetByRole(AriaRole.Heading, new() { Name = "Garfield" }))
                .ToBeVisibleAsync();
        });
    }
}
