...
- Use `IQueryable<T>` judiciously and understand when queries execute
...
- Avoid SQL injection by using parameterized queries (LINQ is safe; `FromSqlRaw` requires care)
- Implement appropriate data access permissions
- Be careful with raw SQL queries — use `FromSqlInterpolated` or parameters
- Consider data encryption for sensitive information

## Testing

- Use in-memory database provider for fast unit tests
- Create separate testing contexts with SQLite or TestContainers for integration tests
